...
export function maxIndependentSubset(graph, vertices) {
  ...
  const maximum_clique = _.maxBy(maximal_cliques, (clique) => clique.length) ?? [];
...
export function maxTwoColorableSubgraphs(graph, vertices) {
  ...
  for (const vs1 of cliques1) {
    ...
    let cliques2 = bronKerbosch(complement(graph, vertices_rest), vertices_rest);
